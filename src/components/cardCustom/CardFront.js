...
const CardFront = ({
  ...
  return (
    <div className="cardBody" style={{ background: backgroundColor ? backgroundColor : (colorPicker.key === "image") ? `url(${colorPicker.value})` : colorPicker.value }} onClick={handleSwipeLeft}>
      {
      ...
      <div className="cardItem" onClick={onShowInfo} style={{ transform: AlingMent(alignMent) }}>
        ...
        {isEnableFront && <div style={{ color: textColor, fontFamily: FontFamily }} className='cardName'>{nameCard}</div>}
      </div>
    </div>
  )
}
